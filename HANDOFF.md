...
## ✅ LATEST ROUND — installable + offline PWA + SEO, DONE
Branch `feat/pwa-offline` (off `main`); committed `5afda8c`, all gates green locally,
awaiting user push + PR. Completes the "on-device/in-browser" thesis. **Zero new deps**
(icons via transitively-present sharp).

- **Service worker** (`web/public/sw.js`, hand-written runtime caching, no precache manifest):
  network-first HTML w/ offline shell fallback, SWR for `_next/static`, cache-first `/mediapipe/*`;
  **bypasses `/model/*` (IndexedDB owns it), `/metrics/*`, and cross-origin ORT CDN**. Cache keyed
  by build SHA via `?v=`. `lib/sw-cache.ts` = pure shouldCache/cacheName the SW mirrors (parity-tested).
- **Registration** (`components/service-worker-register.tsx`): prod-only via a **HOSTNAME guard**
  (skips localhost), NOT NODE_ENV — because Playwright runs a *production* build, so an env guard
  would register the SW under e2e and corrupt the inference no-refetch test. Hostname guard keeps
  the test origin SW-free.
- **Manifest** (`app/manifest.ts` typed/force-static, dark theme_color) + 192/512/maskable PNG icons
  (`public/icons/`, generated by `scripts/gen-icons.mjs`) + `app/apple-icon.tsx` + viewport themeColor.
- **SEO**: `app/sitemap.ts`, `app/robots.ts`, schema.org WebApplication JSON-LD (`lib/structured-data.ts`)
  inlined in `<head>` with the **honest 59.8%/96.9%** numbers (no fake aggregateRating).

Gates: tsc, lint, **145 unit** (+18), build emits all PWA/SEO files, **21 e2e** (incl. the inference
no-refetch test STILL green — proves the SW doesn't touch /model), Lighthouse perf/a11y ≥0.90.
**Out of scope (flagged):** self-hosting the ORT wasm for a fully-offline FIRST visit (today ORT's
CDN fetch is the one thing a brand-new offline visitor can't get; shell + MediaPipe + model all work
offline after first visit). **PWA round is the honest stopping point** — further web work needs a new
external input (multi-signer dataset / J/Z video benchmark) or is polish the audits rated not-worth-it.

---

## Earlier round — dark/light theme toggle, MERGED as PR #21 (squash `89acafa`)
The previously-twice-deferred high-risk feature. CSS-var migration + AA-verified light palette +
dual-theme axe gate. **Merged and live.**
